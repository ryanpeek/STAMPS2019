...
**For code:** *[See tutorial on RStudio Notebooks](https://rmarkdown.rstudio.com/lesson-2.html)*

This notebook demonstrates many R Notebook features. Ignore this first chunk for now; we'll come back to it.

```{r setup, include=FALSE}

knitr::opts_chunk$set(echo = TRUE) # this sets default for chunks to always be echo=TRUE
# load libraries for all chunks in document
library(ggplot2)
library(dygraphs)
library(mapview)
...
Rmarkdown is a really great way to write notes, reports, even books or manuscripts if you are willing. The biggest difference is that you can type as you like anywhere in the document, and you can **only** run code by setting it apart with a code chunk, which looks kind of funny:

![](rchunk.png)

The most important component is the three backtics (` ``` `) and the squiggly brackets with an r (` {r} `).

## Output Types

RMarkdown provides a number of possible outputsFirst, let's create a simple code chunk that prints the numbers from 1 to 10. Run this chunk with *Ctrl+Shift+Enter* (*Cmd+Shift+Enter* on OS X). 

```{r numbers, eval=TRUE, echo=TRUE}
numbers <- seq_len(15)
numbers
```
...
```{r graphics, echo=T, eval=T}
ggplot() + 
  geom_point(data=iris, aes(x=Sepal.Length, y=Petal.Length, color=Species, size=Petal.Width), pch=21)
```
